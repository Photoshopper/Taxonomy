...
class VocabularyController extends AdminBaseController
{
    ...
    {
        $vocabulary_id = $this->vocabulary->create($request->all());

        return redirect()->route('admin.taxonomy.term.index', [$vocabulary_id])
            ->withSuccess(trans('core::core.messages.resource created', ['name' => trans('taxonomy::vocabularies.title.vocabulary')]));
    }
     ...
     * @return Response
     */
    public function update(Vocabulary $vocabulary, CreateVocabularyRequest $request)
    {
        $this->vocabulary->update($vocabulary, $request->all());

        return redirect()->route('admin.taxonomy.vocabulary.index')
            ->withSuccess(trans('core::core.messages.resource updated', ['name' => trans('taxonomy::vocabularies.title.vocabulary')]));
    }
    ...
    public function destroy(Vocabulary $vocabulary)
    {
        ...
        return redirect()->route('admin.taxonomy.vocabulary.index')
            ->withSuccess(trans('core::core.messages.resource deleted', ['name' => trans('taxonomy::vocabularies.title.vocabulary')]));
    }
}
